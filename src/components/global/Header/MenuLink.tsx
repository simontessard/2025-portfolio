...
export default function MenuLink({href, text, hideMenu} : {href: string, text: string, hideMenu: () => void}) {
    ...
    return (
        <Link onClick={hideMenu} href={href} className={`relative group overflow-hidden text-white tracking-tight font-primary py-2 uppercase text-4xl md:text-5xl xl:text-7xl size-fit`}>
            {text}
            {pathname === href &&
