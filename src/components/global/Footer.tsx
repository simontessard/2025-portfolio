            ...
            <div className="flex flex-col gap-6 md:gap-4 mb-6 md:mb-8">
                <div className="flex items-start justify-between w-full">
                    ...
                    <div className="flex flex-col">
                        <div className="flex max-md:flex-col flex-wrap md:items-center gap-y-4 xl:gap-y-8 gap-x-6 md:gap-x-8 xl:gap-x-7 w-full mb-4">
...
function FooterLink({href, text}) {
    ...
    return (
        <Link href={href} className={`relative group size-fit overflow-hidden text-primary tracking-tight font-primary uppercase text-3xl md:text-4xl xl:text-5xl 2xl:text-6xl`}>
            {text}
            {pathname === href &&
                <span className="absolute top-0 bottom-0 my-auto bg-primary block h-1 xl:h-1.5 w-full"/>
            }
            {pathname != href &&
                <span className="absolute top-0 bottom-0 -left-full my-auto bg-primary block h-1 xl:h-1.5 w-full md:group-hover:translate-x-full transition-transform duration-300"/>
            }
        </Link>
    )
}